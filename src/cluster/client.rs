            ...
            loop {
                ...
                match msg {
                    Some(SyncResponse::AuthorityAction(_, action)) => {
                        if let Err(e) = inner.state.lock().await.apply_authority(action).await {
                            tracing::error!("failed to apply authority action from follow feed: {e}");
                        }
                    }
