    ...
    D::Action: Clone,
{
    ...
    async fn subscribe_fresh(&self) -> bool {
        let Ok((state, mut feed)) = ({
            ...
            tracing::warn!("failed to subscribe peer to fresh state stream");
            self.inner.fail("failed to subscribe peer to fresh state stream");
            return false;
        };
    ...
    async fn handle_action(&self, source: A, action: D::Action) {
        ...
        if is_leader {
            ...
            if let Err(error) = self
                ...
                .await
            {
                tracing::error!(?error, "failed to apply forwarded authority action");
                self.inner
                    .fail(format!("failed to apply forwarded authority action: {error:?}"));
            }
            return;
        }
