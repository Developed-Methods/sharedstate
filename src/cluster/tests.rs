...
use crate::{
    cluster::{
        client::{recv_timeout, send_expect_ok, PROTOCOL_VERSION},
        election::{valid_local_leader_path, valid_remote_leader_path, NodeTiming},
        messages::{SharePeerDetails, SyncRequest, SyncResponse},
        node::{NodeDebugInfo, NodeState},
...
async fn high_latency_cluster_converges_with_configured_timing() {
    ...
    let mut node3 = start_cluster_node_with_timing(&net, 3, false, &[1, 2], timing, io_settings).await;
