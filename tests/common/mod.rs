//! Real TCP wiring shared by the integration tests, mirroring the kv_tui
//! example.
...
pub async fn start_node(io: LocalhostTcpIo, can_lead: bool, peers: &[u16]) -> SharedState<LocalhostTcpIo, KvState> {
    start_node_with_gateway(io, can_lead, None, peers).await
}

pub async fn start_node_with_gateway(
    io: LocalhostTcpIo,
    can_lead: bool,
    voter_gateway: Option<u16>,
    peers: &[u16],
...
pub async fn wait_for_value(node: &SharedState<LocalhostTcpIo, KvState>, key: &str, value: &str, timeout: Duration) {
...
pub async fn wait_for_settled_leader(
